...
import { toggleBlockUserAction, deleteUserAction, editUserAction } from './actions';
      ...
      <aside className="w-64 bg-festara-navy text-festara-sand flex flex-col hidden md:flex">
        ...
        <div className="p-4 border-t border-white/10">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-full bg-festara-gold flex items-center justify-center text-festara-navy font-bold">
              ADM
            </div>
            <div>
              <p className="text-sm font-medium">Admin</p>
              <p className="text-xs text-red-400">Accès Total (Root)</p>
            </div>
          </div>
        </div>
      </aside>
                ...
                {filteredTxns.map(txn => (
                  <li key={txn.id} className="p-6 hover:bg-gray-50 transition-colors flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs font-medium text-gray-400 mt-2">{txn.time}</p>
                    </div>
                  </li>
                ))}
              </ul>
